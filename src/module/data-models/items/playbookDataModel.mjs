...
export default class PlaybookDataModel extends BaseItemDataModel {
  ...
  async getItems() {
    ...
    if (stale.length > 0) {
      console.warn(`GFV1: Stale references in ${this.parent.name}: ${stale.join(", ")} will be deleted`);
  ...
  async addRef(uuid) {
    ...
    if (!["rule", "asset", "bond", "identity", "strain"].includes(item.type))
      throw new Gfv1Error(`Playbook doesn't currently support importing ${item.type}`);
    if (item.system.playbookType !== undefined && item.system.playbookType !== this.playbookType) {
  ...
  async deleteRef(uuid) {
    ...
    if (index === -1) throw new Gfv1Error(`Cannot delete ${uuid} from playbook. Not Found`);
    const items = this.items;
  ...
  async reorderRefs(orderedRefs) {
    ...
    if (orderedRefs.length !== indexes.length) throw new Gfv1Error("Invalid reorder");
