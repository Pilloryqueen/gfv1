...
export default async function migrateWorld(oldVersion) {
  ui.notifications.info(`Applying migrations for GIRLFRAME version ${game.system.version}. Please wait...`, {
    permanent: true,
  });

  await runMigrations(oldVersion);

  ui.notifications.info(`Migrations for GIRLFRAME version ${game.system.version} completed!`, { permanent: true });
}
