...
class DragFlow {
  ...
  async drop(event) {
    ...
    if (this.target.dataset.itemUuid === undefined) {
      ...
      const targetElement = this.listElement.querySelector("li:not(.drag-target, .drag-moving)");
      return {
        sortedItems,
...
class DropFlow {
  ...
  async internalDrop(event, dragFlow) {
    ...
    if (!f) return console.warn(`GFV1 | ${this.sheet.constructor.name} has no drop handler for internal`);
    return f.bind(this.sheet)(await dragFlow.drop(event));
  }

  async externalDrop(event) {
    const { documents, type } = await DocumentHelper.getDocumentsFromDropEvent(event);
    const f = this.dropHandlers[type.toLowerCase()];
    if (!f) return console.warn(`GFV1 | ${this.sheet.constructor.name} has no drop handler for ${type}`);
