...
export default class DialogHelper {
  ...
  static async selectImport({ items, type, preSelect }) {
    ...
    const content = await renderTemplate("systems/gfv1/templates/dialog/select.hbs", context);
    try {
  ...
  static async warn_playbook_import(play, playbook) {
    const context = {
      ...
      playbook_type: game.i18n.localize(GFV1.playbooks[playbook.system.playbookType]),
    };
    const content = game.i18n.format("GFv1.dialog.warnPlaybookImport.content", context);
    const buttons = [
      {
        action: "fix",
        default: true,
      ...
      {
        action: "abort",
        label: game.i18n.format("GFv1.dialog.warnPlaybookImport.abort", context),
        callback: abort,
      },
      {
        action: "continue",
        label: game.i18n.format("GFv1.dialog.warnPlaybookImport.continue", context),
      },
    ];
  ...
  static async rollModifierQuery({ item, actor }) {
    ...
    const content = await renderTemplate("systems/gfv1/templates/dialog/rollModifier.hbs", context);
    return await foundry.applications.api.DialogV2.prompt({
      window: {
  ...
  static async _queryNumber(options) {
    ...
    const content = await renderTemplate("systems/gfv1/templates/dialog/queryNumber.hbs", o);
    try {
      return await foundry.applications.api.DialogV2.prompt({
        ...
        ok: {
          label: o.confirm,
          callback: (event, button, dialog) => button.form.elements.num.valueAsNumber,
        },
      });
    } catch {
      return undefined;
