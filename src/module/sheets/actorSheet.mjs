...
export default class Gfv1ActorSheet extends HandlebarsApplicationMixin(ActorSheetV2) {
  static ACTIONS = {};
  ...
  async _prepareContext(options) {
    ...
    context.description = await this.document.enrichedDescription(this.document.isOwner);
  ...
  async _importPlaybook(playbook) {
    ...
    return this.actor.importPlaybook(playbook, [...identities, ...assets, ...bonds, ...rules, ...strains]);
  }
  ...
  static async _onEditImage(event, target) {
    if (target.nodeName !== "IMG") {
      throw new Gfv1Error("The editImage action is available only for IMG elements.");
    }
