import { preloadedTemplates } from "../../handlebars/preload.mjs";
import { deleteDoc, embraceTag, makeRoll, rollStrain, viewDoc } from "./itemControl.mjs";
import ItemProperty from "./itemProperty.mjs";

export default class ListItem {
  constructor(dataModel, item) {
    this.dataModel = dataModel;
    this.item = item;
  }

  render({ actor, locked, playbookType, editable }) {
    const context = {
      item: this.item,
      uuid: this.item.uuid,
      tooltip: this.item.system.tooltip,
      properties: this.dataModel.itemListProperties.map(p => new ItemProperty(this.dataModel, this.item, p)),
      type: this.dataModel.type,
      controls: controls,
      actor,
      locked,
      playbookType,
      editable,
    };
    return preloadedTemplates.itemListEntry(context);
  }
}

const controls = [makeRoll, rollStrain, embraceTag, viewDoc, deleteDoc];
